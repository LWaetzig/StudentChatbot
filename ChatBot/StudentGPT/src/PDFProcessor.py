...
class PDFProcessor:
    ...
    def extract_text(
        self, page: fitz.fitz.Page, extract_from_image: bool = False
    ) -> str:
    ...
    def extract_images(self, page: fitz.fitz.Page, save_images=False, **kwargs) -> list:
    ...
    def process_pdf(self, **kwargs) -> dict:
        ...
        Args:
            **kwargs: extract_from_image, save_images, path
        ...
        extract_from_images = (
            False
            if "extract_from_image" not in kwargs
            else kwargs["extract_from_image"]
        )
        ...
        for i, page in enumerate(self.document):
            ...
            text = self.extract_text(page, extract_from_image=extract_from_images)
            images = self.extract_images(page, save_images=save_images, path=path)
